...
# In[23]:
## Scatter plot comparison for dataset 1
...
## Scatter plot comparison for dataset 2
fig = plt.figure(figsize=(12, 6))
for modelname, Model, params_list in models:
        for params in params_list:
...
for modelname, Model, params_list in models:
    for params in params_list:
        # Fit model to the training set for dataset 1
        model = Model(**params)
        model.fit(X_train, y_train) 
        y_pred = model.predict(X_test)
        ...
        ## Plot predicted vs true for dataset 1 
        ...
        ## Plot predicted vs residuals for dataset 1
...
for modelname, Model, params_list in models:
    for params in params_list:
        # Fit model to the training set for dataset 2
        model = Model(**params)
        model.fit(X_train2, y_train2) 
        ...
        ## Plot predicted vs true for dataset 2 
        fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))
        ...
        ## Plot predicted vs residuals for dataset 2
        ax[1].scatter(y_pred, residuals, color="red")
        ax[1].vlines(x=max_pred, ymin=0, ymax=max_error, color='black', linestyle='--', alpha=0.7, label="max error")
        ax[1].grid(True)
        ax[1].set(xlabel="Predicted", ylabel="Residuals", title= "Predicted vs Residuals")
