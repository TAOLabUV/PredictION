...
## for dataset 1 models comparison
...
# In[21]:
# for dataset2 models comparison
for modelname, Model, params_list in models:
    for params in params_list:
        # Fit model to the training set
        model = Model(**params)
...
# In[23]:
## K fold validation for dataset2
scores = []
cv = model_selection.KFold(n_splits=5, shuffle=True, random_state = seed)
for modelname, Model, params_list in models:
    for params in params_list:
        ...
        plt.title('%s: %s' %(Model.__name__,'K-Fold Validation'))
        plt.legend()
        plt.savefig('figs3' + Model.__name__ + '.svg')
        plt.show()
print(scores)
...
plt.ylabel('True')
plt.legend()
plt.savefig('comparison_dtf2.svg')
plt.show()
...
# In[26]:
## Residuals and error plots dataset 2
from statsmodels.graphics.api import abline_plot
...
for modelname, Model, params_list in models:
    for params in params_list:
        ...
        fig.suptitle(Model.__name__, fontsize=13)
        plt.subplots_adjust(wspace=0.2)
        plt.savefig('figs5' + Model.__name__ + '.svg')
        plt.show()
        ...
        ax.grid(axis='x')
        ax.set(title= '%s: %s' %(Model.__name__, "Residuals distribution"))
        plt.savefig('figs7' + Model.__name__ + '.svg')
        plt.show()
